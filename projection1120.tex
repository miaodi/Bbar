...
\added{Contour plots of the error in $\sigma_{xx}$ for biquartic elements of the most refined mesh are shown in Figure \ref{fig:platewithhole_contour}, We can see from these plots that $Q_4$ elements have severe error while the projection methods all reduce error to a more acceptable level. Here we can also see that the non-symmetric method has a slight improvement compared to the symmetric method.}
\begin{figure}[h]
    \center
\begin{tabular}{ccc}
...
We have demonstrated the performance of the approach in the context of shear deformable beams (to alleviate transverse shear locking) and nearly incompressible elasticity problems (to alleviate volumetric locking). The proposed method reduces locking errors and achieves (nearly) optimal convergence rates \added{ for the symmetric method and optimal rates for the non-symmetric method}. The cases where optimal rates were not achieved \added{when using the the symmetric formulation are a symptom of the fact that the symmetric formulation has may not satisfy the inf-sup condition}.
