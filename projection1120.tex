...
To integrate \Bezier projection into a standard finite element assembly algorithm, it is convenient to recast \Bezier projection in terms of a dual basis. A dual basis has the distinguishing property that
\begin{align}
...
\added{The relationship of $\bar{B}$-formulation with mixed formulations has been studied and proved to be equivalent by Simo and Hughes \cite{hughes_variational_1986}. However, the symmetric \Bezier $\bar{B}$ projection lacks the connection to the mixed formulation. For this reason, instead of considering $\bar{B}$ formulation as a strain projection method, we regard it as a mixed formulation with the auxiliary variable ( the shear stress for the case of Timoshenko beam) being eliminated and use the \Bezier dual basis functions as the test functions of the auxiliary variable. Since the resulting stiffness matrix is not symmetric, this formulation is refered to as non-symmetric \Bezier $\bar{B}$ projection.}
\subsubsection{The weak form}
...
\end{equation}
\added{Compared to \eqref{eq:symmetric_timoshenko}, the formulation of stiffness matrix contributed by shear strain requires one less matrix operation in \eqref{eq:nonsymmetric_timoshenko}. One can note that, due to the different choices of function spaces of shear stress and its variation, the global stiffness matrix is no longer symmetric.}
...
\added{The contour plots of $\sigma_{xx}$ for bicubic elements of the third mesh are shown in Figure \ref{fig:platewithhole_contour}, $Q_3$ elements are severely locked while all $\bar{B}$ methods converge to the reference solution. }
\begin{figure}[h]
    \center
\begin{tabular}{ccc}
\includegraphics[width=.33\linewidth]{deformation_p_2} & \includegraphics[width=.33\linewidth]{deformation_p_3} & \includegraphics[width=.33\linewidth]{deformation_p_4}\\
\includegraphics[width=.33\linewidth]{stress_p_2} & \includegraphics[width=.33\linewidth]{stress_p_3} & \includegraphics[width=.33\linewidth]{stress_p_4}\\
