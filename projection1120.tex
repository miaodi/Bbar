...
\added{In this paper we introduce two methods that employ \Bezier projection to increase the sparsity of the stiffness matrix that results form $\bar{B}$ methods, which typically lead to a dense system. We call these methods the symmetric and non-symmetric \Bezier $\bar{B}$ projection methods based on the resulting symmetry of the stiffness method. We show that both methods result in a sparse stiffness matrix, reduce locking, and result in optimal (non-symmetric method) or near optimal (symmetric method) convergence rates.  We also perform an inf-sup analysis to help provide further insight into the behavior and quality of these methods.}
...
\end{align}
where \added{$N_A$ is a degree $p$ spline basis function, $\omega_A$ and $\phi_A$ are the corresponding control point values.}. \added{The shear strain and bending strain can then be expressed as}
...
\begin{equation}
    ...
    \end{bmatrix},
\end{equation}
\added{where both the shear stress and its variation are discretized by the dual basis functions. However, for the inner product of dual basis functions }
\begin{equation}
...
Following the same pattern as the non-symmetric \Bezier $\bar{B}$ method for the Timoshenko beam problem, we keep the discretization of $\mathbf{u}$ and $\mathbf{w}$ untouched, while using lower order spline basis functions and corresponding dual basis functions for the discretization of pressure $p$ and its variation $\delta{p}$, respectively. The discretized stiffness matrix in mixed form can be written as
\begin{equation}
    \mathbf{K}_{mix}=
    \begin{bmatrix}
...
\end{equation}
We note, that in contrast to the symmetric \Bezier $\bar{B}$ method, the assembly of the stiffness matrix in this case can be performed in the standard way with no need for global matrix operations.
...
Convergence plots for the relative error of the displacement and energy in the $L^2$ norm are shown in Figure \ref{fig:platewithhole_convergence}. As can be seen, the standard $Q_p$ approximations suffer from severe volumetric locking for all orders while, on the other hand, the $\bar{B}$ methods remedy locking for all cases. \added{For the symmetric \Bezier $\bar{B}$ method, the optimality in all three measures has been obtained for biquadratic elements; the optimal energy convergence rate has been achieved for bicubic elements; however, the degradation of convergence rates in all three measures has been observed for biquartic elements. The can be attributed to the fact that the derivation of the symmetric \Bezier $\bar{B}$ method is purely based on the engineering analogy between the $L^2$ projection and \Bezier projection. On the other hand, the non-symmetric \Bezier $\bar{B}$ method, derived from the mixed formulation, is optimal for displacement, stress and energy for all elements with slightly higher errors than that of global $\bar{B}$ method, which agrees with the inf-sup test.}\par

\added{Contour plots of  $\sigma_{xx}$ for biquartic elements of the finest mesh are shown in Figure \ref{fig:platewithhole_contour}. We denote that, for the finest mesh, results from all $\bar{B}$ methods are consistent with the refernce solution, while using standard finite element still leads to a huge error. Figure \ref{fig:platewithhole_error_contour} shows the absolute error of $\sigma_{xx}$ with the same mesh. As can be seen, the projection methods reduce error to an acceptable level (less than $.1\%$ of the maximal $\sigma_{xx}$) while the error the $Q_4$ element is at the same level of the maximal $\sigma_{xx}$. Here we can also see that the non-symmetric method has a slight improvement compared to the symmetric method.}
\begin{figure}[h]
    \center
\begin{tabular}{ccc}
...
\begin{figure}[htb!]
    ...
    Reference & Legend
    \end{tabular}
        \caption{ Contour plots of $\sigma_{xx}^h$ for the plate with a circular hole ($p=4$, and the finest mesh is used).}
