...
\added{Contour plots of  $\sigma_{xx}$ for biquartic elements of the finest mesh are shown in Figure \ref{fig:platewithhole_contour}. We note that, for the finest mesh, results from all $\bar{B}$ methods are consistent with the reference solution, while using standard finite element results in large errors in the stress. Figure \ref{fig:platewithhole_error_contour} shows the absolute error of $\sigma_{xx}$ with the same mesh. As can be seen, the projection methods reduce error to an acceptable level (less than $.1\%$ of the maximal $\sigma_{xx}$) while the error the $Q_4$ element is at the same level of the maximal $\sigma_{xx}$. Here we can also see that the non-symmetric method has a slight improvement compared to the symmetric method.}
\begin{figure}[h]
    \center
\begin{tabular}{ccc}
...
We have demonstrated the performance of the approach in the context of shear deformable beams (to alleviate transverse shear locking) and nearly incompressible elasticity problems (to alleviate volumetric locking). The proposed method reduces locking errors and achieves (nearly) optimal convergence rates \added{ for the symmetric method and optimal rates for the non-symmetric method}. The cases where optimal rates were not achieved \added{when using the the symmetric formulation are a symptom of the fact that the symmetric formulation is not directly related to a variational principle so it may not satisfy the inf-sup condition}.
